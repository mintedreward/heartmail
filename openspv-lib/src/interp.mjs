 ...
 * components of HeartMail Lib because of the giant switch statement in step().
    ...
    if (fExec && opCodeNum >= 0 && opCodeNum <= OpCode.OP_PUSHDATA4) {
      ...
      switch (opCodeNum) {
        ...
        case OpCode.OP_NUM2BIN: {
          ...
          const arg1 = this.stack[this.stack.length - 1]
          const n = new Bn().fromScriptNumBuffer(arg1, fRequireMinimal)

          if (n.lt(0) || n.gt(2147483647)) {
            this.errStr = 'SCRIPT_ERR_PUSH_SIZE'
            return false
          }

          const size = n.toNumber()
          // This library will never validate transactions before genesis.
          // Therefore the following code is disabled.
          //
          ...
          this.stack.pop()
          let rawnum = this.stack[this.stack.length - 1]

          // Try to see if we can fit that number in the number of
          // byte requested.
          const num = new Bn().fromSm(rawnum, { endian: 'little' })
          try {
            rawnum = num.toSm({ endian: 'little', size })
          } catch (err) {
        ...
        case OpCode.OP_BIN2NUM: {
          ...
          let rawnum = this.stack[this.stack.length - 1]
          const n = new Bn().fromSm(rawnum, { endian: 'little' })
          this.stack[this.stack.length - 1] = n.toSm({ endian: 'little' })

          // The resulting number must be a valid number.
          if (this.stack[this.stack.length - 1].length > MaxScriptNumLength) {
            this.errStr = 'SCRIPT_ERR_INVALID_NUMBER_RANGE'
            return false
          }
        }
  ...
  * results (scriptSig, scriptPubKey, tx, nIn, flags, valueBn) {
  ...
  getDebugObject () {
    ...
    return {
      ...
      opCodeStr: this.script && this.script.chunks && this.script.chunks[pc]
