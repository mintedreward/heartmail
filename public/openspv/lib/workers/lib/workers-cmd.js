...
let inject = function (deps) {
  ...
  class WorkersCmd extends Struct {
    constructor (objbuf, classname, methodname, args, id) {
      super({objbuf, classname, methodname, args, id})
    }
    ...
    static argsToBw (bw, args) {
      ...
      for (let index in args) {
        ...
        if (Buffer.isBuffer(arg)) {
          ...
          bw.write(arg)
        } else if (arg instanceof Struct || arg instanceof Bn) {
          ...
          bw.write(Buffer.from(classname))
          ...
          let buf = Buffer.from(JSON.stringify(arg))
          bw.writeVarIntNum(buf.length)
          bw.write(buf)
        }
      }
    ...
    static serialize (args) {
      let bw = new Bw()
      bw = WorkersCmd.argsToBw(bw, args)
      return bw.toBuffer()
    }

    static deserialize (buf, classes) {
      let br = new Br(buf)
      let args = WorkersCmd.argsFromBr(br, classes)
      return args
    }
  }
