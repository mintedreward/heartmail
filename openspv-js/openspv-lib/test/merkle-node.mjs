...
describe('MerkleNode', function () {
  ...
  describe('hash', function () {
    it('should hash these buffers', function () {
      const merkle1 = new MerkleNode(undefined, Buffer.alloc(0))
      const merkle2 = new MerkleNode(undefined, Buffer.alloc(0))
      const merkle = new MerkleNode(undefined, undefined, merkle1, merkle2)
    ...
    it('should hash this buffer', function () {
      const merkle = new MerkleNode(undefined, Buffer.alloc(0))
  ...
  describe('#fromBuffers', function () {
    it('should find this merkle root from three buffers', function () {
      ...
      const merkle = new MerkleNode().fromBuffers(bufs)
      const hashBuf = merkle.hash()
      hashBuf.length.should.equal(32)
      hashBuf
        .toString('hex')
  ...
  describe('@fromBuffers', function () {
    it('should find this merkle root from three buffers', function () {
      const bufs = [Buffer.alloc(0), Buffer.alloc(0), Buffer.alloc(0)]
      const merkle = MerkleNode.fromBuffers(bufs)
      const hashBuf = merkle.hash()
      hashBuf.length.should.equal(32)
      hashBuf
        .toString('hex')
  ...
  describe('#fromBufferArrays', function () {
    it('should find this merkle root from two buffers', function () {
      ...
      const merkle = new MerkleNode().fromBufferArrays(bufs1, bufs2)
      const hashBuf = merkle.hash()
      hashBuf.length.should.equal(32)
    })
  ...
  describe('@fromBufferArrays', function () {
    it('should find this merkle root from two buffers', function () {
      const bufs1 = [Buffer.alloc(0)]
      const bufs2 = [Buffer.alloc(0)]
      const merkle = MerkleNode.fromBufferArrays(bufs1, bufs2)
      const hashBuf = merkle.hash()
      hashBuf.length.should.equal(32)
    })
